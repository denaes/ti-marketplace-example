...
description: >
  Guide product managers through structured PRD (Product Requirements Document) creation by
  orchestrating problem framing, user research synthesis, solution definition, and success criteria
  into a cohes
metadata:
  type: skill
  department: product
...
## Ask before writing (mandatory)

Before writing any section of the PRD, **ask the user as many clarifying questions as possible** (problem, users, success metrics, scope, constraints, priorities). Do not make decisions in place of the user. Do not write substantial content until key questions are answered or the user explicitly asks for a draft (then ask at least the most critical questions first). See `standards/conventions.md` (Ask before deciding). If the feature relates to **existing product or code** (e.g. TI platform), **search the codebase** (`ti/`, `ti/v3/`) for the area involved, **surface current state to the user**, then define the solution so it respects existing architecture and rules. See `standards/conventions.md` (Codebase-first).
...
### TI repo templates
- **PRD template:** `templates/product/Product Brief [TEMPLATE].md` — Source of truth for all document templates: `docs/templates/`
