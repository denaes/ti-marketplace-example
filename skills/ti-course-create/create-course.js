#!/usr/bin/env node
/**
 * Thought Industries Incoming API — create course(s).
 *
 * POST {base}/incoming/v2/content/course/create
 *
...
function printHelp() {
  ...
  node create-course.js [--base-url=URL] [--api-key=KEY | --api-key-file=PATH] [--file=PATH | --json='...' | -]
...
Options:
  ...
  -                    Read JSON from stdin (same as --file=-)

Input JSON normalization:
  • Full API shape: { "courseAttributes": [ { "title", "kind", ... } ] }
  • Single course object: { "title", "kind", "sections"? | "topics"? | ... } → wrapped as one element
  • Array of course objects → wrapped as courseAttributes
...
function normalizeCreateBody(raw) {
  ...
  if (Array.isArray(raw)) {
    return { courseAttributes: /** @type {JsonObject[]} */ (raw) };
  }

  if ("courseAttributes" in obj) {
    const ca = obj.courseAttributes;
    if (!Array.isArray(ca)) {
      throw new Error('create: "courseAttributes" must be an array of course objects.');
    }
    return { courseAttributes: /** @type {JsonObject[]} */ (ca) };
  }

  if (typeof obj.title === "string" && typeof obj.kind === "string") {
    return { courseAttributes: [obj] };
  }

  throw new Error(
    'create: expected { courseAttributes: [...] }, a course object with "title" and "kind", or an array of course objects.',
  );
}
...
async function loadJsonRaw(path, inlineJson) {
  ...
  if (path == null) {
    if (process.stdin.isTTY) {
      throw new Error("Provide course JSON via --file=path.json, --json='{...}', or pipe JSON on stdin (not a TTY).");
    }
    return readStdinUtf8();
  }
 ...
 * @param {unknown} body
 */
async function postCreate(apiRoot, token, body) {
...
async function main() {
  ...
  await postCreate(apiRoot, token, body);
}
