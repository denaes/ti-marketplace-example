...
When asked to create a PRD, **ask the user as many questions as needed** (problem, goals, scope, constraints, users, success metrics, out-of-scope) **before writing**. Do not assume—ask. Do not produce the full PRD until you have enough input or the user explicitly asks for a draft (then ask at least the most critical questions first). See `standards/conventions.md` (Ask before deciding). If the feature touches existing product/code, search `ti/` and surface current state to the user before proposing the solution. See `standards/conventions.md` (Codebase-first).
